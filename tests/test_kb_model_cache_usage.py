...
class KBModelCacheUsageTests(unittest.TestCase):
    def test_ingestor_uses_cached_sentence_transformer(self):
        fake_faiss = object()
        fake_model = object()
        with patch("src.kb.ingest._load_kb_backends", return_value=(fake_faiss, object)):
            with patch("src.kb.ingest.get_sentence_transformer", return_value=fake_model) as mock_get:
    ...
    def test_searcher_uses_cached_sentence_transformer(self):
        ...
        fake_model = object()
        with patch("src.kb.search._load_kb_backends", return_value=(_FakeFaiss(), object)):
            with patch("src.kb.search.get_sentence_transformer", return_value=fake_model) as mock_get:
